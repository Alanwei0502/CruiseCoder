...
if (isset($_POST["yearMonth"])) {
  ...
  foreach ($phoneTutorialData as $index => $row) {
    ...
    if (isset($_POST["userAccount"])) {
      $userAccount = $_POST["userAccount"];
      $checkBuySql = "SELECT * FROM member AS m join myorder AS o on m.mNumber = o.oMember join invoice AS i on o.oNumber = i.iNumber join course AS c on i.iCourse = c.cNumber WHERE mAccount = '$userAccount' AND icourse = '$theCourse'";

...
if (checkCookie("user")) {
  $("a.complete").prop(
    "href",
    "https://tibamef2e.com/demo-projects/ted101/G2/frontEnd/info.php"
  );
} else {
...
for (let j = 0; j < nextQuestion.length; j++) {
  nextQuestion[j].addEventListener("mousedown", function (e) {
    ...
    if (
      !selections[0].checked &&
      !selections[1].checked &&
      !selections[2].checked &&
      !selections[3].checked
    ) {
      ...
      // 判斷作答是否正確
      let chooseOption = this.closest("section").querySelector(
        ".selection:checked"
      ).value;
      let correctAnswer = this.closest("section")
        .querySelectorAll("div.question")[0]
